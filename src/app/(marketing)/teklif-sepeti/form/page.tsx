...
export default function TeklifFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Hata Oluştu: " + (err.message || "Sunucuya ulaşılamadı."));
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
          ...
          <div className="p-8 md:p-12 border-b border-slate-100 relative overflow-hidden">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase">Ad</label><input required name="ad" value={formData.ad} onChange={handleChange} type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-cyan-500 outline-none transition-all" /></div>
              <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase">Soyad</label><input required name="soyad" value={formData.soyad} onChange={handleChange} type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-cyan-500 outline-none transition-all" /></div>
              <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase">Unvan / Akademik Derece</label><input name="unvan" value={formData.unvan} onChange={handleChange} type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-cyan-500 outline-none transition-all" /></div>
          ...
          <div className="p-8 md:p-12 border-b border-slate-100 bg-slate-900 relative">
            <h2 className="text-2xl font-bold text-white mb-8 flex items-center gap-3">
              ...
              Hizmet Seçimi (En az 1 tane seçiniz)
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {ALL_SERVICES.map((hizmet) => {
                const isSelected = selectedServices.includes(hizmet);
                return (
                  <div 
                    key={hizmet} 
                    onClick={() => toggleService(hizmet)}
                    className={`cursor-pointer flex items-start gap-3 p-4 rounded-xl border transition-all duration-200 select-none ${isSelected ? 'bg-pink-500/20 border-pink-500' : 'bg-slate-800/50 border-slate-700 hover:bg-slate-800'}`}
                  >
                    <div className={`mt-0.5 w-5 h-5 rounded flex items-center justify-center shrink-0 border ${isSelected ? 'bg-pink-500 border-pink-500' : 'border-slate-600'}`}>
                      {isSelected && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
                    </div>
                    <span className={`text-sm font-semibold leading-tight ${isSelected ? 'text-white' : 'text-slate-300'}`}>{hizmet}</span>
                  </div>
                );
              })}
            </div>
